...
open class TrollCommand private constructor(private val command: String, val condition: BooleanReturn, private val imagePaths: Array<String>?, val sound: SoundEvent? = null, private val isSubCommand: Boolean) : Command<CommandSourceStack> {
    ...
    val emptyPayload = EmptyPayload(Identifier.fromNamespaceAndPath(TrollCommands.MOD_ID, "${command}_boolean"))
    ...
    fun register(dispatcher: CommandDispatcher<CommandSourceStack>) {
        dispatcher.register(Commands.literal(command).executes(this).then(Commands.argument(ARGUMENT_KEY, EntityArgument.players()).requires { serverCommandSource -> serverCommandSource.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER) }.executes(asSubCommand)))
    }
    override fun run(context: CommandContext<CommandSourceStack>): Int {
        val players: List<ServerPlayer?> = if (isSubCommand) EntityArgument.getPlayers(context, ARGUMENT_KEY).distinct() else listOf(context.source.player)
        players.forEach {
            if (it != null) ServerPlayNetworking.send(it, emptyPayload)
        }
        return 0
    }
    ...
    open fun command(context: GuiGraphicsExtractor) {
        ...
        if (!imagePaths.isNullOrEmpty()) {
            currentImage?.let {
                ...
                context.blit(RenderPipelines.GUI_TEXTURED, Identifier.fromNamespaceAndPath(TrollCommands.MOD_ID, it), 0, 0, 0F, 0F, width, height, width, height)
            }
        }
